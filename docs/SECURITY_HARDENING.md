...
**Location**: `api/server.py` - `security_headers()` function; tested in `tests/api/test_security_headers.py`.

### 3. Rate Limiting

Two independent layers:

- **Auth endpoints** (`/api/auth/login`, `/register`, `/2fa/*`, OAuth URL/callback/link routes): Flask-Limiter (`api/requirements.txt`), applied per route via the `@auth_rate_limit(...)` decorator in `api/server.py`. Login is limited to 5/minute + 20/hour per IP; 2FA and OAuth routes to 10/minute; registration to 10/hour. `storage_uri="memory://"`, appropriate for this app's single-process deployment -- a multi-worker/gunicorn setup would need `storage_uri` pointed at something shared (e.g. Redis) instead, since in-memory counters aren't shared across processes.
...
Tested in `tests/api/test_auth.py` (`TestSessionCookieHardening`, `TestCsrfProtection`) and `tests/api/test_oauth.py` (state-mismatch cases).
